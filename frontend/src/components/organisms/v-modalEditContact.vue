<template>
	<Teleport to="body">
		<div v-if="(modal.openEditModal = true)" class="modal">
			<div class="addForm">
				<v-input
					class="input"
					:inputType="inputType"
				...
				<!-- <v-select
					...
					class="select"
				/> -->
				<v-button class="submitButton" @click="editContact()">Edit Contact</v-button>
			</div>
			<div class="close">
				<v-button class="closeButton" @click="modal.openEditModal = false">x</v-button>
			</div>
		</div>
	</Teleport>
</template>
...
const editContact = () => {
	if(contact.form.firstname === "" && contact.form.lastname === "" && contact.form.middlename==="") {
		...
		contact.editContact(contact.form.id ?? "", contact.form.firstname ?? "", contact.form.middlename ?? "", contact.form.lastname ?? "", contact.form.phoneNumber ?? "", contact.form.email ?? "", contact.form.organization ?? "");
	}
}
</script>
